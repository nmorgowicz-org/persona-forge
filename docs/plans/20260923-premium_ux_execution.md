...
| **The deck's speed knob ignores pointer drags.** Reproduced by hand in the browser: a real drag on `[data-testid="deck-speed"]` leaves `aria-valuenow` at `1`, while the **identical component** in the DSP row hand-drags 100 → 214 in the same build. So it is the deck's *context*, not the knob. `drag-scrub.spec.js:131` has been red since B-P5 (the card that introduced the knob) and I mis-read that phase's verification as all-green — "8 passed" was 6 knob + 2 drag-scrub, with three drag tests failing above the `tail` I read. Exonerated by measurement: `size` (tested at 44, still inert) and B-P6's `data-help` listener (disabled and rebuilt, still inert). Not fully inert: clicking the value still opens the typed editor. The failing test is left failing rather than weakened. | B-P5 → found at B-P7 | **next, before B-P8** |
