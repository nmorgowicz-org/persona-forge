...
| **Two playback-exclusivity tests fail deterministically when `tests/ui/core/` is run as a unit, and pass in the full suite.** `core/exclusive-audition.spec.js:175` ("previewing a prosody variant stops when the lane compare starts") and `core/transport.spec.js:64` ("the Speak deck and the Voice Edit A/B lane each report themselves as the active source") both failed twice with `--retries=0` (`30 passed / 2 failed / 1 skipped` out of 33 listed) and passed with retries. Both concern the same two owners (the prosody preview and the voice-edit lane) from A-4b/A-9, so this is a plausible real race in the playback coordinator, not test noise — and the full suite hides it behind `retries: 2` (the config sets them when `CI` is set). | B-P7 verification | **next, before B-P8** — it guards a shipped invariant |
| The full-suite summary reports fewer tests than `--list` counts (142 passed + 1 skipped vs 152 listed). `core/` alone shows **no** gap (33 listed = 30 + 2 + 1), so the difference lives in another directory and is most likely tests that passed only on retry (reported as `flaky`, above the lines my `tail` showed). Not yet confirmed; it does not affect any gate result, since every run has been 0-failed. | B-P7 verification | P10, with the flake above |
