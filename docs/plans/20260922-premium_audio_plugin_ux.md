...
| 3b S3 completion (hover readout + scrub) | PASS 2026-09-23 | 6479ec4 | S3's acceptance prose taken on by owner decision: one `formatHoverTime` + one `useHoverTimeGuide` across all five waveform surfaces (deck, clip lane, prosody variant lanes, prosody region editor, voice-edit compare; the stitch ruler adopted it and dropped its bespoke guide), plus Alt-drag scrub on the deck. waveform-pointer.spec 6/6; full suite 82/82. Readout precision is now 1dp below 10s everywhere (was 2dp/3dp on two surfaces) — single-grammar consequence, one-line change if the owner wants finer. Follow-up refactor a1cc047 (lane scale measured at hover time, not observed). |
| 4 M1 loop brace + N6 | | | |
