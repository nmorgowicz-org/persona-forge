...
| `TTS_MAX_SPEECH_SECONDS` | Preset-specific (e.g. 300) | qwen3-tts-engine-only (pytorch/openvino); `pocket_tts` never reads this and is unbounded. For `openvino`, baked into the IR at export time — changing it requires re-export. For `pytorch`, read at process start — changing it requires a container restart. |
| `QWEN3_ENGINE_MAX_NEW_TOKENS` | `800` | qwen3-tts-engine-only. Base token ceiling for a single generation, before the CPU hang-avoidance clamp below is applied. |
| `QWEN3_ENGINE_CPU_MAX_NEW_TOKENS` | `300` | qwen3-tts-engine-only. Hang-avoidance cap applied whenever pytorch or openvino runs on CPU (no iGPU deployment has been validated yet) — both are too slow to safely decode without it. Independent of `TTS_MAX_SPEECH_SECONDS`; the tighter of the two always wins. |
| `QWEN3_ENGINE_CPU_BF16_MAX_NEW_TOKENS` | `160` | qwen3-tts-engine-only. Extra-tight cap for `pytorch` + `bfloat16`, a combination known to hang or diverge on many CPUs. |
...
| `ACCEL_TORCH_INDEX_URL` | per-family manifest pin (`.../whl/xpu`, `.../whl/cu126`, `.../whl/rocm7.1`) | Wheel index for the first-boot torch/torchaudio install when the family is not `cpu`. Sourced from `persona_forge/accelerator_manifest.py`; override if the default is wrong for your hardware. |
| `ACCEL_TORCH_VERSION` | per-family manifest pin (`intel-xpu` 2.13.0 / `cuda` 2.14.0 / `rocm` 2.13.0) | torch version for the first-boot install. |
