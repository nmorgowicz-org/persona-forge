...
| P10 residue + verdict | PASS 2026-09-24 (20 A-0 scenarios + lookdev re-run, all exit 0; suite green) | b165398 | Residue: lab copy deleted, two debug logs deleted, `rounded-4xl` already migrated, no placeholder names. Scorecard: **no FAIL cells**, every cell PASS or N/A-with-reason, each tagged to a capture in `docs/screenshots/artifacts/` (legend and the spec-backed cells spelled out in R §5). A-2's long-carried `zoom-hover` flake is diagnosed and fixed (Fit derives its px/s from the plan's total duration, which was still growing as the clip analysis resolved; the test now waits for the level to settle). One escalation (fake-tier version string in published shots) and one deferral (`AudioDeck` split — needs a behaviour card, not this copy-only one). Details in R §7. |
| P11 docs + media | PASS 2026-09-24 (11/11 README images regenerated this phase; capture self-test 15/15) | 7759884, fdf477e | The single publishing re-shoot, executed against the **real** backend rather than the fake tier: every README image regenerated from a live `pocket_tts` process with the deployment's own default voice, so the storefront shows real measurements and a real version (`v1.5.0+dev.7759884`) instead of placeholders and `v0.0.0-fake`. New `signal-layer-gif` (meter + playhead + wave/spectrum under playback) embedded in the README hero block, plus the Signal Crucible `app-hero.png` banner, a §4 Controls reference in `FRONTEND_OVERVIEW.md`, and two drift corrections there. One scenario (`prosody-adjustment`) was broken against the current UI on *both* sources and is fixed; its final capture subject is still wrong and is recorded as such. Details, gate method and the engine-pin note in R §7. |
| P12 archive + PR | archive PASS 2026-09-24; PR #315 opened on owner approval | ad1882c | Five planning docs archived to `docs/archive/luminous-instrument/` (stamped, history preserved via `git mv`); 24 inbound references rewritten across 17 files; active `docs/plans/` left clean apart from unrelated older plans and the AudioDeck follow-up. PR draft written (title, Release Please override block with the breaking first entry, scorecard, capture index) and shown to the owner — the card stops there. |
