...
| P12 archive + PR | archive PASS 2026-09-24; PR draft awaiting owner | ad1882c | Five planning docs archived to `docs/archive/luminous-instrument/` (stamped, history preserved via `git mv`); 24 inbound references rewritten across 17 files; active `docs/plans/` left clean apart from unrelated older plans and the AudioDeck follow-up. PR draft written (title, Release Please override block with the breaking first entry, scorecard, capture index) and shown to the owner — the card stops there. |
