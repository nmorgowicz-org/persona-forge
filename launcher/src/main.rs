...
//! env via bundled `uv`, (4) exec the installed `persona-forge` console script with whatever
//! arguments the user passed straight through (`doctor`, `setup`, `serve`, ...). Never logs
...
fn run() -> Result<ExitCode, String> {
    ...
    let console_script = bootstrap::console_script(&env_dir);
    let args: Vec<String> = env::args().skip(1).collect();

    exec_console_script(&console_script, &args)
}

#[cfg(unix)]
fn exec_console_script(console_script: &std::path::Path, args: &[String]) -> Result<ExitCode, String> {
    use std::os::unix::process::CommandExt;
    let err = std::process::Command::new(console_script).args(args).exec();
    Err(format!("failed to exec {}: {err}", console_script.display()))
}

#[cfg(not(unix))]
fn exec_console_script(console_script: &std::path::Path, args: &[String]) -> Result<ExitCode, String> {
    let status = std::process::Command::new(console_script)
        .args(args)
        .status()
        .map_err(|e| format!("failed to spawn {}: {e}", console_script.display()))?;
